...
from services.flight_manager import book_flight

project = "gemini-flights-414423"
...
book_flight_declaration = generative_models.FunctionDeclaration(
    name = "book_flight",
    description= "Tool for booking a flight with flight number, seat type, and number of seats",
    parameters={
        "type": "object",
        "properties": {
            "flight_id": {
                "type": "string",
                "description": "The flight ID associated with the flight id."
            },
            "seat_type": {
                "type": "string",
                "description": "The type of seat in the flight to book such as economy, first class, etc."
            },
            "num_seats": {
                "type": "integer",
...
def handle_response(response):
    ...
    if response.candidates[0].content.parts[0].function_call.args:
        ...
        if "book_flight" in response.candidates[0].content.parts[0].function_call.name:
